...
class GestionClient(tk.Frame):
    ...
    def afficher_clients(self):
        ...
        for c in clients:
            self.tree.insert('', 'end', values=(
                c.get_id_client(),
                c.get_type_client() if hasattr(c, 'get_type_client') else '',
                c.get_nom() or '',
                c.get_prenom() or '',
    ...
    def ajouter_client(self):
        ...
        def maj_champs(event=None):
            """Affiche les champs selon le type sélectionné"""
        ...
        def valider():
            ...
            cle = DAOClient.get_instance().insert_client(nouveau)
    ...
    def modifier_client(self):
        ...
        # values : (ID, Type, Nom, Prénom, Raison Sociale, Email, Téléphone, Statut)

        popup = tk.Toplevel(self)
        popup.title("Modifier un client")
        popup.geometry("420x380")
        popup.resizable(False, False)
        popup.transient(self)
        popup.grab_set()

        ttk.Label(popup, text="Modifier le client", font=('Arial', 14, 'bold')).pack(pady=15)

        form_frame = ttk.Frame(popup)
        form_frame.pack(padx=20, pady=10, fill='both', expand=True)

        champs = [("Nom :", values[2]), ("Prénom :", values[3]),
                  ("Raison Sociale :", values[4]), ("Email :", values[5]), ("Téléphone :", values[6])]
        entries = []
        for i, (label, valeur) in enumerate(champs):
            ...
            e.insert(0, valeur if valeur else '')
            e.grid(row=i, column=1, pady=5, padx=5)
            entries.append(e)

        ttk.Label(form_frame, text="Statut :").grid(row=len(champs), column=0, sticky='w', pady=5)
        combo_statut = ttk.Combobox(form_frame, width=28, state='readonly')
        combo_statut['values'] = ('PROSPECT', 'CLIENT', 'ANCIEN')
        combo_statut.set(values[7])
        combo_statut.grid(row=len(champs), column=1, pady=5, padx=5)
        ...
        def valider():
            ...
            modifie = Client(
                values[0],
                entries[0].get().strip() or None,
                entries[1].get().strip() or None,
                entries[2].get().strip() or None,
                None,
                entries[4].get().strip() or None,
                entries[3].get().strip(),
                combo_statut.get()
            )
            succes = DAOClient.get_instance().update_client(modifie)
            if succes:
                tk.messagebox.showinfo("Succès", "Client modifié avec succès !")
                popup.destroy()
                self.afficher_clients()
            else:
                tk.messagebox.showerror("Erreur", "Impossible de modifier le client")
    ...
    def supprimer_client(self):
        ...
        values = self.tree.item(selection[0])['values']
        id_client = values[0]
        nom_affiche = values[2] or values[4]  # nom ou raison sociale
        ...
        if reponse:
            from domaine.Client import Client
            c = Client(id_client, None, None, None, None, None, None, None)
