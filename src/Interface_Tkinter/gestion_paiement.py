...
class GestionPaiement(tk.Frame):
    def __init__(self, parent, utilisateur, on_back=None):
        ...
        ttk.Button(frame_gauche, text="👁️ Voir les paiements",
        ...
        ttk.Button(frame_gauche, text="✏️ Modifier le paiement",
                   command=self.modifier_paiement).pack(pady=5, fill='x')
       
        if utilisateur['role'] == 'ADMIN':
            ttk.Button(frame_gauche, text="🗑️ Supprimer paiement",
    ...
    def faire_paiement(self):
        ...
        facture_selectionnee = {'numero': None}
        ...
        def choisir_facture():
            ...
            def confirmer():
                ...
                facture_var.set(f"{valeurs[0]} ({valeurs[2]})")
                popup_fact.destroy()
    ...
    def modifier_paiement(self):
        ...
        ttk.Label(form_frame, text="Facture :").grid(row=0, column=0, sticky='w', pady=5)
        facture_var = tk.StringVar(value=facture_actuelle)
        ttk.Label(form_frame, textvariable=facture_var, foreground='blue').grid(
            row=0, column=1, sticky='w', pady=5, padx=5)
        facture_selectionnee = {'numero': facture_actuelle}
        ...
        def choisir_facture():
            ...
            factures = DAOFacture.get_instance().select_facture()
            for f in factures:
                tree_fact.insert('', 'end', values=(f.get_numero_facture(), f.get_date_emission(), f"{f.get_montant_total()} €", f.get_etat()))
